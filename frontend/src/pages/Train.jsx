...
const PIPELINE_STEPS = [
  ...
  "Train & persist Isolation Forest",
];
  ...
  return (
    <>
      <PageHeader
        ...
        description="Train an Isolation Forest on the latest uploaded training dataset."
        action={
          <button className="button-primary" disabled={loading} onClick={train}>
            <BrainCircuit className={`h-5 w-5 ${loading ? "animate-pulse" : ""}`} />
          ...
          <div className="mt-6 space-y-5">
            {PIPELINE_STEPS.map((step, index) => (
              <div className="flex gap-4" key={step}>
                <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-cyan/30 bg-cyan/10 text-xs font-bold text-cyan">
                  {index + 1}
                </span>
                <div>
                  <p className="text-sm font-medium text-slate-200">{step}</p>
                  {index < PIPELINE_STEPS.length - 1 && (
                    <div className="ml-[-33px] mt-3 h-3 border-l border-slate-700" />
                  )}
                </div>
              </div>
            ))}
        ...
        <article className="panel min-h-96 p-6 lg:col-span-3">
          {!metrics ? (
            <div className="flex h-full min-h-80 flex-col items-center justify-center text-center">
              ...
              <h2 className="font-semibold text-slate-300">No training metrics yet</h2>
              <p className="mt-2 max-w-sm text-sm text-slate-500">
                Upload a dataset, then start training to see model performance.
              </p>
            </div>
          ) : (
            <>
              <div className="mb-6 flex items-center gap-3">
                ...
                <div>
                  <h2 className="font-semibold text-white">Training complete</h2>
                  <p className="text-xs text-slate-500">
                    {metrics.dataset_name} • {metrics.feature_count} features •{" "}
                    evaluated using {metrics.evaluation}
                  </p>
                </div>
              </div>
              ...
              <div className="grid max-w-sm grid-cols-2 gap-2 text-center text-sm">
                <div className="rounded-lg bg-cyan/10 p-4">
                  <b className="block text-xl text-cyan">{metrics.confusion_matrix[0][0]}</b>
                  <span className="text-xs text-slate-500">True normal</span>
                </div>
                <div className="rounded-lg bg-red-500/10 p-4">
                  <b className="block text-xl text-red-300">{metrics.confusion_matrix[0][1]}</b>
                  <span className="text-xs text-slate-500">False anomaly</span>
                </div>
                <div className="rounded-lg bg-amber-500/10 p-4">
                  <b className="block text-xl text-amber-300">{metrics.confusion_matrix[1][0]}</b>
                  <span className="text-xs text-slate-500">Missed anomaly</span>
                </div>
                <div className="rounded-lg bg-cyan/10 p-4">
                  <b className="block text-xl text-cyan">{metrics.confusion_matrix[1][1]}</b>
