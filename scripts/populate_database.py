...
def post_file(endpoint, filepath, label):
    ...
    with filepath.open("rb") as fh:
        r = requests.post("%s/%s" % (BASE, endpoint), files={"file": (filepath.name, fh, "text/csv")}, timeout=300)
...
def post_file_bytes(endpoint, filename, content_bytes, label):
    ...
    r = requests.post("%s/%s" % (BASE, endpoint), files={"file": (filename, content_bytes, "text/csv")}, timeout=300)
    if r.status_code >= 400:
        print("  [WARN] HTTP %d: %s" % (r.status_code, r.text[:200]))
        return {}
    data = r.json()
...
# UNSW-NB15-style training dataset (relabelled 0/1)
unsw_train_bytes = make_unsw_csv(training_file)
unsw_train_result = post_file_bytes("upload", "UNSW-NB15_training.csv", unsw_train_bytes, "UNSW-NB15 Training Dataset")
dataset_id_2 = unsw_train_result.get("id")
...
# ---------------------------------------------------------------------------
# Step 3 -- Evaluate (already done during training, stored in TrainingRun)
# ---------------------------------------------------------------------------
...
with pred_file.open("rb") as fh:
    r = requests.post("%s/predict" % BASE, files={"file": (pred_file.name, fh, "text/csv")}, timeout=300)
if r.status_code < 400:
    d = r.json()
    print("  OK: dataset_id=%s total=%s anomalies=%s severity=%s" % (
...
# UNSW-NB15-style prediction dataset
print("\n--- Step 5: Run Prediction on UNSW-NB15-style Dataset ---")
unsw_pred_bytes = make_unsw_csv(pred_file)
print("\n[PREDICT] UNSW-NB15_prediction.csv")
r2 = requests.post("%s/predict" % BASE, files={"file": ("UNSW-NB15_prediction.csv", unsw_pred_bytes, "text/csv")}, timeout=300)
...
if isinstance(dash, dict):
    ...
    for entry in act:
        print("    - %s: records=%s anomalies=%s" % (entry.get("name"), entry.get("records"), entry.get("anomalies")))
...
if isinstance(history, list):
    ...
    for h in history:
        print("    ID=%-3s | %-50s | %-10s | rows=%-8s | preds=%s" % (
            h.get("id"), h.get("dataset_name", "")[:50], h.get("purpose"), h.get("record_count"), h.get("prediction_count")))
...
if isinstance(alerts, list):
    ...
    for a in alerts[:10]:
        print("    ID=%-3s | %-40s | row=%-5s | %s" % (
            a.get("id"), a.get("dataset_name", "")[:40], a.get("row_number"), a.get("severity")))
    if len(alerts) > 10:
